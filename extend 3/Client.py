...
from tkinter import Button, Label, messagebox, W, E, N, S
...
class Client:
	...
	def getDescribe(self):
		"""Get more information of media stream"""
		if self.state != self.INIT:
		...
		while True:
			try:
				...
				if data:
					...
					print("Received frame", currFrameNumber)

					if currFrameNumber > self.frameNbr:
						self.frameNbr = currFrameNumber
						self.updateMovie(self.writeFrame(rtpPacket.getPayload()))

			except:
				#stop listening when receive pause or teardown request
				if self.playEvent.isSet():
					break
				#close rtp socket when receive teardown request
	...
	def recvRtspReply(self):
		...
		while True:
			respone = self.rtspSocket.recv(1024)

			if respone:
				self.parseRtspReply(respone)
	...
	def parseRtspReply(self, data):
		...
		seqNumber = int (dataLines[1].split(' ')[1])
	...
	def openRtpPort(self):
		...
		try:
			...
			messagebox.showwarning('Unable to bind PORT={}').format(self.rtpPort)
